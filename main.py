...
def main(yolo):
    ...
    vs = cv2.VideoCapture('test_data.mp4')
    writer = None
    (W, H) = (None, None)
    ...
    while True:
        ...
        draw.text((150,6), 'car %d \nmotor %d \nbus %d \n' %(car,motor,bus), fill=(0, 0, 0),font=font)
        ...
        cv2.namedWindow("result for ECL", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("result for ECL", 904, 544)
        cv2.imshow('result for ECL',frame)
